<template>
  <h1 class="dark-title mt-5 mb-5 fs-2 text-center">Edite sua notícia</h1>
  <form class="form-noticia box dark-body-text">
    <div class="group column-direction">
      ...
      <input
        type="text"
        id="title"
        class="w-100 boring-gray-border form-control"
    ...
    <div class="group column-direction ms-4 dates">
      ...
      <div class="group row-direction">
        <div class="d-flex flex-wrap align-center">
          <input
            type="radio"
            id="confirm-chosen-date"
            class="boring-gray-border"
          ...
          <div class="time ms-3">
            <input
              type="time"
              name="hora-publicacao"
              id="hora-publicacao"
              class="boring-gray-border"
              v-model="horaPublicacao"
              disabled
              required
            />
          </div>
...
<script setup lang="ts">
  ...
  const handleSubmit = async () => {
    ...
    sendingNews.value = await noticiaStore.putNews(noticia.value)
  ...
  onMounted(async () => {
    if (props.noticiaId) {
      await noticiaStore.getNewsById(props.noticiaId)
      noticia.value = noticiaStore.response.dado
      console.log(noticia.value)

      dataPublicacao.value = noticia.value.dataPublicacao.split("T")[0]
      horaPublicacao.value = noticia.value.dataPublicacao.split("T")[1]
    }
  })
</script>
