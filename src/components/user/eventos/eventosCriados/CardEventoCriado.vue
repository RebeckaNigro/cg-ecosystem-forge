    ...
    <div class="container-fluid box p-3 my-3 h-100 overflow-hidden">
      <div v-if="!props.isRascunho">
        <img
          ...
          alt="event-image"
          class="image-limiter"
          :class="{ 'evento-encerrado': encerrado }"
        />
      </div>
      <div v-else>
      ...
      <p
        class="fs-4 titulo-evento mt-2 text-start p-2"
        :class="{ 'texto-evento-encerrado': encerrado }"
      >
...
const confirmDelete = () => {
  ...
  confirmStore.showConfirmModal('Tem certeza que desesja remover este evento?', props.evento.id);
};
...
onMounted(() => {
  ...
  if (props.evento.ultimaAtualizacao) {
    dataAtualizacaoFormatada.value = ` ${friendlyDateTime(props.evento.ultimaAtualizacao)} `;
  }
