	...
	<div>
		<div class="container-fluid box p-3 my-3 h-100 overflow-hidden">
			<div v-if="!props.isRascunho">
				<img :src="evento.arquivo
						? 'data:image/png;base64, ' + evento.arquivo
						: '/eventos/eventoExpandido/default-event-cover.svg'
					" alt="event-image" class="image-limiter" :class="{'evento-encerrado' : encerrado}"/>
			</div>
			<div v-else>
				...
				<div class="row mt-2 mx-2 justify-content-between align-items-center d-flex">
					...
					<div class="col-6 col-lg-4 text-end p-0" v-if="!props.isRascunho">
						<img src="/view_icon.svg" alt="" class="image-icon-button" @click="
							...
							" />
						<img src="/edit_icon.svg" alt="" class="image-icon-button" @click="
							$router.push({
								name: 'GerenciaEvento',
			...
			<p class="fs-4 titulo-evento mt-2 text-start p-2" :class="{'texto-evento-encerrado' : encerrado}">
			...
			<div class="text-start mt-4">
				<span class="text-secondary px-1" v-if="evento.ultimaAtualizacao">Atualizado em: {{ dataAtualizacaoFormatada
				}}</span>
			</div>
		</div>
		<ConfirmModal v-show="confirmStore.visible" @confirm-true="confirmado" element-id="confirmEventModal"
