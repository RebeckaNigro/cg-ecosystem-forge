  ...
  <form @submit.prevent="cadastrar" class="container box p-5 my-5">
    ...
    <div class="text-start mb-3">
      ...
      <span
        ...
        <a href="/politica-privacidade.pdf"> Termos de Uso e Políticas de Privacidade. </a>
      </span>
    </div>
    ...
    <div class="row mt-5">
      ...
      <div class="col">
        <button type="submit" class="col green-btn-primary">CADASTRAR</button>
      </div>
    </div>
  </form>
</template>
...
const formularioRules = computed(() => {
  return {
    ...
    confirmPassword: {
      required,
      sameAs: helpers.withMessage('Senhas não conferem.', sameAs(perfil.value.password)),
    },
  };
});
...
watch(perfil.value, () => {
  if (perfil.value.cpf) perfil.value.cpf = CPFMask(perfil.value.cpf);

  if (perfil.value.cep) perfil.value.cep = CEPMask(perfil.value.cep);
});
onMounted(async () => {
  getInstituicoes();
