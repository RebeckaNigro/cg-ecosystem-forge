  ...
  <div class="text-start">
    ...
    <div>
      ...
      >
        <option value="">
          {{
            type == 'evento' || type == 'documento' || type == 'eventoOrganizador'
              ? 'Todos'
              : 'Todas'
          }}
        </option>
...
const doFilter = async (event: any) => {
  if (event.target.value) {
    ...
    if (props.type == 'noticia') {
      ...
      dataReferencia.setDate(dataReferencia.getDate() - Number(selectedOption.value));
      dataReferencia.setHours(0, 0, 0);

      results.value = props.items.filter((noticia: NoticiaSimplificada) => {
        return adjustStringDateForTimezone(noticia.dataPublicacao).getTime() >=
      ...
      results.value = props.items.filter((evento: EventoSimplificado) => {
        return adjustStringDateForTimezone(evento.dataInicio).getTime() >= dataReferencia.getTime()
          ? true
          : false;
      });
      sendSearchResult();
