  ...
  <div class="autocomplete">
    ...
    <ul class="autocomplete-results" v-show="isOpen">
      ...
      >
        ...
        <span v-if="props.type == 'Instituicao'">{{ result.razaoSocial }} </span>
      </li>
    </ul>
  </div>
</template>
...
const filterResults = () => {
  if (props.type == 'CustomTag') {
    results.value = props.items.filter(
      (item: any) => item.descricao.toLowerCase().indexOf(search.value.toLowerCase()) > -1
    );
  } else if (props.type == 'Instituicao') {
    results.value = props.items.filter(
      (item: any) => item.razaoSocial.toLowerCase().indexOf(search.value.toLowerCase()) > -1
    );
  }
};
