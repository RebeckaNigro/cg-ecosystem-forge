    ...
    <div class="row w-100 justify-content-between">
      <div class="col-12 col-md-6 filter-container">
        <FilterComponent
          ...
          :items="items"
          type="parceiro"
          @filter-result="filtrarParceiros"
        />
      </div>
  ...
  <section class="container-cards-parceiros" v-for="(item, index) in partners" :key="index">
    <header>
      <div @click="item.toggle = !item.toggle" class="d-flex hover-pointer">
        ...
        <img
          src="/parceiros/arrow-down.svg"
          alt="Mostrar parceiros"
          class="align-self-start mx-2"
          :class="item.toggle ? 'reverse' : ''"
        />
      </div>
      <hr class="w-100" />
    </header>
    <main v-if="item.toggle">
      ...
      <CardParceiro
        v-for="(parceiro, index) in item.parceiros"
        :key="index"
        :card-parceiro="parceiro"
      />
    </main>
  </section>
</template>
...
const filtrarParceiros = (parceirosFiltrados: Array<IPartnerSeccionado>) => {
