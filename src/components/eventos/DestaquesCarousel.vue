        ...
        <section class="d-flex infos mt-2 pt-4 ps-5 pb-4 pe-5 align-items-center">
          <main>
            <h1
              id="event-name"
              class="dark-title"
              :class="{ 'texto-evento-encerrado': checaEventoEncerrado(data) }"
            >
              {{ checaEventoEncerrado(data) ? data.titulo + ' (ENCERRADO)' : data.titulo }}
            </h1>
            <time class="light-body-text calendar-icon">{{
              data.dataInicio ? brDateString(data.dataInicio) : ''
            }}</time>
